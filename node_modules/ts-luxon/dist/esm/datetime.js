...
class DateTime {
  ...
  static _buildObject(config, units, defaultValues, objNow) {
    ...
    const gregorian = config.useWeekData ? weekToGregorian(config.normalized) : config.containsOrdinal ? ordinalToGregorian(config.normalized) : config.normalized, [tsFinal, offsetFinal] = objToTS(gregorian, config.offsetProvis, config.zoneToUse), inst = new DateTime({
      ts: tsFinal,
